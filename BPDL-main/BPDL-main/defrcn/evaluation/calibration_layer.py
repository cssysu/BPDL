...
class PrototypicalCalibrationBlock:
    ...
    def execute_calibration(self, inputs, dts):
        ...
        for i in range(ileft, iright):
            ...
            if dts[0]['instances'].scores[i] >= 0.7:
                flag = True
                if tmp_class not in features_temp:
                    features_temp[tmp_class] = []
                features_temp[tmp_class].append(torch.tensor(features[i-ileft].cpu().data.numpy().reshape((1, -1))))
        if flag:
            prototypes = {}
            temp = []
            for label in features_temp:
                features = torch.cat(features_temp[label],dim=0)
                prototypes[label] = torch.mean(features, dim=0, keepdim=True)
                kl = F.kl_div(F.log_softmax(prototypes[label], dim = -1), F.softmax(self.prototypes[label].cpu().data, dim = -1), reduction='batchmean')
                if(kl < 1):
                    temp.append(prototypes[label])
                    temp.append(self.prototypes[label].cpu().data)
                    feature = torch.cat(temp, dim=0)
                    self.prototypes[label]= torch.mean(feature, dim=0, keepdim=True)
        return dts
