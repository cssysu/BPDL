...
class FastRCNNOutputLayers(nn.Module):
    ...
    def __init__(
        ...
        num_bbox_reg_classes1 = 1
        self.bbox_pred1 = nn.Linear(input_size, num_bbox_reg_classes1 * box_dim)
    ...
    def forward(self, x, y, flag=False):
        if flag:
            ...
            proposal_deltas = self.bbox_pred1(x)  # box
            scores = self.cls_score(x)  # class predict
        else:
            if x.dim() > 2:
                x = torch.flatten(x, start_dim=1)
            if y.dim() > 2:
                y = torch.flatten(y, start_dim=1)
            proposal_deltas = self.bbox_pred(y)  # box
            scores = self.cls_score(x)  # class predict
