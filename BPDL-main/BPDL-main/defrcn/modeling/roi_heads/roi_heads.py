...
class Res5ROIHeads(ROIHeads):
    ...
    def __init__(self, cfg, input_shape):
        ...
        self.cov = nn.Conv2d(2048, 2048, 1)
    ...
    def forward(self, images, features, proposals, targets=None):
        ...
        box_features = self.cov(box_features)
        feature_pooled = box_features.mean(dim=[2, 3])  # pooled to 1x1

        pred_class_logits_yuan, pred_proposal_deltas = self.box_predictor(
            feature_pooled, feature_pooled, flag = True
        )
        
        del feature_pooled
        # print("box:", pred_proposal_deltas)
        box_type = type(proposals[0].proposal_boxes)
        num_preds_per_image = [len(p) for p in proposals]
        proposals_temp = box_type.cat([p.proposal_boxes for p in proposals])
        num_pred = len(proposals_temp)
        
        B = proposals_temp.tensor.shape[1]
        K = pred_proposal_deltas.shape[1] // B
        pred_boxes = self.box2box_transform.apply_deltas(
            pred_proposal_deltas.view(num_pred * K, B),
            proposals_temp.tensor.unsqueeze(1)
                .expand(num_pred, K, B)
                .reshape(-1, B),
        )
        pred_boxes = pred_boxes.view(num_pred, K * B).split(
            num_preds_per_image, dim=0
        )
        boxes = []
        ...
        cls_feature = self._shared_roi_transform(
            [features[f] for f in self.in_features], boxes
        )
        cls_pooled = cls_feature.mean(dim=[2, 3])  # pooled to 1x1

        feature_pooled_s = F.relu(self.fc_s(cls_pooled))
        feature_pooled_l = F.relu(self.fc_l(cls_pooled))
        ...
        if self.training:
            ...
            storage = get_event_storage()
            if int(storage.iter) >= 300:
                gt_classes = outputs.gt_classes
                bg_class_ind = pred_class_logits.shape[1] - 1
                true_cases = (gt_classes >= 0) & (gt_classes < bg_class_ind)
                loss_kl = F.kl_div(F.log_softmax(pred_class_logits_yuan[true_cases], dim=-1),
                                   F.softmax(pred_class_logits[true_cases],dim=-1) , reduction='batchmean')
                losses.update({'loss_kl': loss_kl * 0.5})
